...
- List query(String sql, RowMapper rowMapper)
	 : 여러 개의 레코드를 반환하는 select계열의 쿼리문인 경우 사용
- List query(String sql, Object[] args, RowMapper rowMapper)
	: 인파라미터를 가진 여러 개의 레코드를 반환하는 select계열의 쿼리문인 경우 사용

- Object queryForObject(String sql, RowMapper rowMapper)
	: 하나의 레코드를 반환하는 select계열의 쿼리문 실행 시 사용된다.
- Object queryForObject(String sql, Object[] args, RowMapper rowMapper)
	: 인파라미터가 있고 하나의 레코드를 반환하는 select계열의 쿼리문 실행 시 사용된다.
	
- int queryForInt(String sql)
	: 쿼리문의 실행 결과가 숫자를 반환하는 select계열의 쿼리문에 사용된다.
- int queryForInt(String sql, Object[] args)
	: 인파라미터가 있고 쿼리문의 실행 결과가 숫자를 반환하는 select계열의 쿼리문에 사용된다.
...
public class JDBCTemplateDAO {
	...
	*/
	...
	public void close() {
		...
		다시 new를 통해 생성해야 하므로 자원해제는 하지 않는다. */
	}
	...
	public void write(final SpringBbsDTO springBbsDTO) {
		...
		반드시 final로 선언하여 값의 변경이 불가능하게 해줘야 한다.
		...
		*/
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				...
				리스트에서 게시물 정렬 시 bgroup을 통해 order by절을 구성하기 때문이다.
				또한 nextval은 한 문장에서 여러 번 사용해도 같은 시퀀스를 반환한다.
				*/
	...
	public SpringBbsDTO view(String idx) {
		...
		/*
		queryForObject()는 반환결과가 0개이거나 2개 이상인 경우
		예외가 발생되므로 반드시 예외처리를 해주는 것이 좋다.
		*/
		try {
